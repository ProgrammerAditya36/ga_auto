...
def check_repo_exists(username, repo_name, token):
    if (os.system(f"gh repo view {username}/{repo_name}")) == 0:
        return True
...
def main():
    ...
    if command == "init":
        ...
        username = os.popen("gh auth status").read().split()[7]
        os.system("git init")
        online_repo = None
        if ("private" in args[1]):
            os.system(f"gh repo create {args[2]} --private --confirm")
            online_repo = args[2].split("/")[-1]
        else:
            os.system(f"gh repo create {args[1]} --public --confirm")
            online_repo = args[1].split("/")[-1]
